...
class MADDPGManager:
    ...
    def remember(
        ...
        obs = stack_obs(obs_dict, self.agent_ids)  # (N, D)
        ...
        rewards = np.asarray([float(rewards_dict.get(aid, 0.0)) for aid in self.agent_ids], dtype=np.float32)
    ...
    def train(self) -> dict[str, float] | None:
        ...
        rewards_t = torch.as_tensor(rewards_b, dtype=torch.float32, device=self.device)  # (B,N)
        ...
        dones_next_t = torch.as_tensor(dones_next_b.astype(np.float32), dtype=torch.float32, device=self.device)  # (B,N)
        active_t = torch.as_tensor(active_b.astype(np.float32), dtype=torch.float32, device=self.device)  # (B,N)
        action_mask_t = torch.as_tensor(action_mask_b, dtype=torch.bool, device=self.device)  # (B,N,A)
        ...
        with torch.no_grad():
            ...
            for i, aid in enumerate(self.agent_ids):
                ...
                a = agent.masked_gumbel_softmax(logits, mask, tau=self.gumbel_tau, hard=True)  # (B,A)
        ...
        for i, aid in enumerate(self.agent_ids):
            ...
            q = agent.critic(state_t, joint_action_onehot)  # (B,)
            with torch.no_grad():
                q_next = agent.target_critic(next_state_t, joint_next_action)  # (B,)
                y = rewards_t[:, i] + (self.gamma * (1.0 - dones_next_t[:, i]) * q_next)

            # Critic Update (MSE Loss)
            # L(θ_i) = 1/S * Σ (y_j - Q_μi(x_j, a_j))^2
        ...
        for i, aid in enumerate(self.agent_ids):
            agent = self.agents[aid]
            logits = agent.actor(obs_t[:, i, :])  # (B,A)
            mask = action_mask_t[:, i, :]  # (B,A)
            a = agent.masked_gumbel_softmax(logits, mask, tau=self.gumbel_tau, hard=True)  # (B,A)
        ...
        for i, aid in enumerate(self.agent_ids):
            ...
            joint_curr = torch.cat(parts, dim=-1)  # (B,N*A)
            
            # Actor Update (Deterministic Policy Gradient)
            # ∇_θi J ≈ 1/S * Σ ∇_ai Q_μi(x_j, a_j) * ∇_θi μ_θi(o_j)
            q_pi = agent.critic(state_t, joint_curr)  # (B,)
