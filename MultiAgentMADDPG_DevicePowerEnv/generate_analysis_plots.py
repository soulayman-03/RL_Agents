...
def plot_privacy_trust_mapping(impact_data, out_path):
    """Génère la cartographie qui lie la confidentialité d'une couche au score de confiance du serveur choisi."""
    # On isole les données des 500 derniers épisodes (quand l'agent est entraîné)
    recent_data = impact_data[-min(500, len(impact_data)):]
    
    privacy_levels = []
    trust_scores = []
    
    for row in recent_data:
        if row.get("ep_failed"): 
            continue # Optionnel: on ne regarde que les stratégies gagnantes
        for alloc in row.get("allocations", []):
            priv = int(alloc.get("layer_privacy", 0))
            trust = float(alloc.get("trust_score", 0.0))
            privacy_levels.append(priv)
            trust_scores.append(trust)
            
    if not privacy_levels:
        print("Aucune donnée valide pour le plot Privacy vs Trust.")
        return

    plt.figure(figsize=(9, 6))
    sns.violinplot(x=privacy_levels, y=trust_scores, palette="Blues", inner="quartile")
    plt.axhline(y=0.7, color='red', linestyle='--', linewidth=1.5, label='Seuil critique de Trust (0.7)')
    plt.title("Stratégie de Sécurité : Privacy Level vs Trust Score attribué", fontsize=14, fontweight="bold")
    plt.xlabel("Niveau de Confidentialité requis par la Couche (Privacy Level)", fontsize=12)
    plt.ylabel("Score de Confiance du Terminal choisi (Trust Score)", fontsize=12)
    plt.legend()
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig(out_path, dpi=300)
    plt.close()
    print(f"[2/3] Privacy/Trust mapping chart saved to {out_path}")
...
def main():
    ...
    plot_privacy_trust_mapping(data, os.path.join(plots_dir, "privacy_vs_trust.png"))
