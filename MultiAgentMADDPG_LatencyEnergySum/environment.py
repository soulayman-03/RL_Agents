...
class MultiAgentIoTEnvLatencyEnergySum(gym.Env):
    ...
    def _energy_cost(self, t_comp: float, t_comm: float) -> float:
        """
        Energy = Power * Time (E = P * T)
        - alpha_comp: Power consumed during calculation (Watts)
        - alpha_comm: Power consumed during transmission (Watts)
        """
        return (self.alpha_comp * float(t_comp)) + (self.alpha_comm * float(t_comm))
    ...
    def get_valid_actions(self) -> Dict[int, List[int]]:
        ...
        for agent_id in range(self.num_agents):
            ...
            for device_id in range(self.num_devices):
                ...
                # Energy hard constraint (Physical model: E = P * T)
                cost = self._energy_cost(t_comp_service, t_comm_service)
    ...
    def step(self, actions: Dict[int, int]):
        ...
        for aid in agent_ids:
            ...
            # Energy cost (Physical model: E = P * T)
            cost = float(self._energy_cost(comp_latency, trans_latency))
            ...
            # Reward shaping: -cost + progress bonus to encourage deep execution
