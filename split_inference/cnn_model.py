...
class BigCNN(nn.Module):
    """
    Bigger CNN (Large task for MARL agents).

    - More compute / more parameters than DeepCNN.
    - Still MNIST-shaped input (1x28x28).
    - Kept as a `ModuleList` to match split-inference / layer-wise execution patterns in this repo.
    """
    ...
    def __init__(self):
        ...
        self.layers = nn.ModuleList(
            [
                ...
                nn.Linear(256, 10),  # Layer 6: Output
            ]
        )
...
class BiggerCNN(nn.Module):
    ...
    Still MNIST-shaped input (1x28x28). Uses more channels and deeper head than BigCNN.
    """
...
class HugeCNN(nn.Module):
    """
    Huge CNN (Large+++ task for MARL agents).

    Much heavier than BiggerCNN. For MNIST, this is intentionally over-parameterized.
    """
