...
class MultiAgentIoTEnvLatencyEnergySum(gym.Env):
    """
    Unified multi-agent environment for Multi-Objective optimization (MADDPG):
    min (Ttotal + Etotal)
    ...
    - Trust hard constraint: if device trust < required trust for a layer, allocation fails.

    *** Device-Specific Power Model ***
    Each device has its own power consumption (Watts):
      - power_comp  (P_comp_i): proportional to cpu_speed  (faster CPU => more Watts)
      - power_comm  (P_comm_i): proportional to bandwidth   (wider link  => more Watts)
    Energy per layer:
      E_comp = power_comp_i * t_comp
      E_comm = power_comm_i * t_comm
      E_total = E_comp + E_comm

    Reward (success):
      reward = -(total_latency + energy_cost)
      where total_latency = t_comp + t_comm
    ...
    def __init__(
        ...
        *,
        ...
        self.dvfs_levels = dvfs_levels or [1.0, 0.75, 0.5]
        ...
        # Assign device-specific power (Watts) proportional to hardware.
        # A faster device consumes more power but finishes tasks quicker.
        self._assign_device_powers()
        ...
        self.action_dim = self.num_devices * len(self.dvfs_levels)
        self.action_spaces = {aid: spaces.Discrete(self.action_dim) for aid in range(self.num_agents)}
    ...
    def _assign_device_powers(self) -> None:
        """
        Assign power_comp and power_comm (in Watts) to every device.
        Power scales linearly with hardware speed so that a faster device
        consumes proportionally more energy per second but finishes sooner.

        Formula:
          power_comp_i = base_power_comp * (cpu_speed_i  / base_cpu_speed)
    ...
    def _device_energy_cost(self, device_id: int, t_comp: float, t_comm: float, dvfs_ratio: float = 1.0) -> float:
        """
        Device-specific energy model with DVFS: E = P_i * T
        Dynamic power scales cubically with frequency (p_comp * dvfs_ratio^3 = current power).
        """
        dev = self.resource_manager.devices.get(int(device_id))
        if dev is None:
            # Fallback: use base power if device not found
            p_comp = float(self.base_power_comp)
            p_comm = float(self.base_power_comm)
        else:
            p_comp = float(getattr(dev, "power_comp", self.base_power_comp)) * (float(dvfs_ratio) ** 3)
            p_comm = float(getattr(dev, "power_comm", self.base_power_comm))
        return (p_comp * float(t_comp)) + (p_comm * float(t_comm))
    ...
    def get_valid_actions(self) -> Dict[int, List[int]]:
        ...
        for agent_id in range(self.num_agents):
            ...
            for device_id in range(self.num_devices):
                for dvfs_idx, dvfs_ratio in enumerate(self.dvfs_levels):
                    trans_data = 0.0
                    ...
                    # Calculate latency components for energy check
                    dev_obj = self.resource_manager.devices[device_id]
                    t_comp_service = float(getattr(current_layer, "computation_demand", 0.0)) / (float(getattr(dev_obj, "cpu_speed", 1.0)) * dvfs_ratio)
                    # Note: In get_valid_actions, we assume no queuing for simplicity of the check
                    t_comm_service = 0.0
                    if prev_dev != device_id:
                         t_comm_service = float(trans_data) / float(getattr(dev_obj, "bandwidth", 1.0))

                    # Energy hard constraint — device-specific model with DVFS: E = P_i * T
                    cost = self._device_energy_cost(device_id, t_comp_service, t_comm_service, dvfs_ratio)
                    ...
                    if self.resource_manager.can_allocate(
                        ...
                        action_id = device_id * len(self.dvfs_levels) + dvfs_idx
    ...
    def step(self, actions: Dict[int, int]):
        ...
        for aid in agent_ids:
            ...
            action_idx = int(actions[aid])
            selected_device_id = action_idx // len(self.dvfs_levels)
            dvfs_ratio = self.dvfs_levels[action_idx % len(self.dvfs_levels)]
            ...
            service_comp = float(getattr(current_layer, "computation_demand", 0.0)) / (float(getattr(dev, "cpu_speed", 1.0)) * dvfs_ratio)
            ...
            # Energy cost — device-specific model with DVFS: E = P_i * T
            cost = float(self._device_energy_cost(selected_device_id, comp_latency, trans_latency, dvfs_ratio))

            # ── Snapshot device state BEFORE allocation ──────────────────────
            snap_before = {
                "device_id":        int(selected_device_id),
                "cpu_speed":        float(getattr(dev, "cpu_speed", 0.0)),
                "bandwidth":        float(getattr(dev, "bandwidth", 0.0)),
                "power_comp":       float(getattr(dev, "power_comp", 0.0)),
                "power_comm":       float(getattr(dev, "power_comm", 0.0)),
                "energy_remaining": float(self.device_energy_remaining.get(selected_device_id, 0.0)),
                "energy_init":      float(self.device_energy_init.get(selected_device_id, 0.0)),
                "memory_used":      float(getattr(dev, "current_memory_usage", 0.0)),
                "memory_capacity":  float(getattr(dev, "memory_capacity", 0.0)),
                "trust_score":      float(self.device_trust.get(selected_device_id, 0.0)),
                "privacy_clearance":int(getattr(dev, "privacy_clearance", 0)),
            }
            ...
            if trust_score < trust_required:
                fail = {
                    ...
                    "trust_shortfall": float(max(0.0, trust_required - trust_score)),
                }
                rewards[aid] = -500.0
                self.agent_done[aid] = True
                infos[aid] = {"reward_type": "stall_termination", "success": False, "fail": fail}
            ...
            # ── Snapshot device state AFTER allocation ────────────────────────
            snap_after = {
                "energy_remaining": float(self.device_energy_remaining.get(selected_device_id, 0.0)),
                "memory_used":      float(getattr(dev, "current_memory_usage", 0.0)),
                "energy_consumed":  float(cost),
                "energy_ratio":     float(self._energy_ratio(selected_device_id)),
            }
            ...
            infos[aid] = {
                ...
                "energy_comp":      float((getattr(dev, "power_comp", 0.0) * (dvfs_ratio**3)) * comp_latency),
                "energy_comm":      float(getattr(dev, "power_comm", 0.0) * trans_latency),
                "energy_remaining": float(self.device_energy_remaining.get(selected_device_id, 0.0)),
                # ── Layer info ───────────────────────────────────────────────
                "layer_idx":        int(self.agent_progress[aid]),
                "layer_name":       str(getattr(current_layer, "name", "")),
                "layer_compute":    float(getattr(current_layer, "computation_demand", 0.0)),
                "layer_memory":     float(getattr(current_layer, "memory_demand", 0.0)),
                "layer_output":     float(getattr(current_layer, "output_data_size", 0.0)),
                "layer_privacy":    int(getattr(current_layer, "privacy_level", 0)),
                "trans_data":       float(trans_data),
                "dvfs_ratio":       float(dvfs_ratio),
                # ── Device snapshots ─────────────────────────────────────────
                "device_before":    snap_before,
                "device_after":     snap_after,
                # ── Trust ────────────────────────────────────────────────────
                "trust_required":   float(trust_required),
                "trust_score":      float(trust_score),
                "trust_hard":       True,
