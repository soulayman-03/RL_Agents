...
# Allow running both:
# - as a module:  python -m MultiAgentVDN.train
# - as a script:  python MultiAgentVDN/train.py
if __package__:
    ...
    from MultiAgentVDN.plots import plot_training_trends, plot_per_agent_training_rewards
...
def train():
    ...
    print(f"Starting VDN Multi-Agent Training with {NUM_AGENTS} agents...")
    ...
    for ep in range(EPISODES):
        ...
        while not all(done.values()):
            ...
            for aid in range(NUM_AGENTS):
                if not done.get(aid, False):
                    ...
                    agent_ep_mappings[aid].append(int(actions.get(aid, -1)))

            # Team termination: if any allocation fails, end the episode for all agents.
            any_fail = False
            ...
            done_team = bool(any_fail or all(bool(v) for v in next_done.values()))

            # Next-state valid actions (for masked targets). If team is terminal, keep None.
            next_valid_actions = None if done_team else env.get_valid_actions()
            ...
            # Track both: sum (for MultiAgent-like logs) and mean (scale-stable)
            team_step_sum = float(sum(rewards.values()))
            team_return_sum += team_step_sum
            team_return_mean += team_step_sum / float(NUM_AGENTS)
            ...
            # A small amount of online updates speeds learning
            loss = manager.train()
            if loss is not None:
                losses.append(loss)

        episode_team_rewards.append(team_return_mean)
        episode_team_rewards_sum.append(team_return_sum)
        eps_history.append(manager._unique_agents()[0].eps.epsilon)
        ...
        if (ep + 1) % 50 == 0:
            ...
            for aid in range(NUM_AGENTS):
                mapping_str = " -> ".join([str(d) for d in agent_ep_mappings[aid] if d != -1])
                finished = bool(env.agent_progress.get(aid, 0) >= len(env.tasks.get(aid, [])))
                success_tag = "[SUCCESS]" if (not agent_failed[aid] and finished) else "[FAILED]"
                print(f"  Agent {aid} ({env.model_types[aid]}): {mapping_str} {success_tag}")
                task_layers = env.tasks[aid]
                layer_details = [
                    f"L{getattr(l, 'layer_id', idx)}({getattr(l, 'computation_demand', '?')}c,{getattr(l, 'memory_demand', '?')}m)"
                    for idx, l in enumerate(task_layers)
                ]
                print(f"    Layer sizes: {' | '.join(layer_details)}")
            print("-" * 30)

    base = os.path.join(SAVE_DIR, "vdn")
    manager.save(base)
