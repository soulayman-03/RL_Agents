...
try:
    ...
    from environment import MonoAgentIoTEnv
    from utils import load_and_remap_weights, set_global_seed
from split_inference.cnn_model import SimpleCNN, HugeCNN
...
def plot_execution_strategy(trace, results_dir, num_devices=5, out_name="execution_strategy.png"):
    ...
    # Sort by layer index for safe plotting
...
def train_single_agent():
    ...
    RESULTS_ROOT = os.path.join(SCRIPT_DIR, "results", "resultBigCNN")
    ...
    for e in range(EPISODES):
        ...
        while not done and step < MAX_STEPS:
            ...
            layer_idx = int(info.get("layer_idx", env.progress - 1))
            if layer_idx >= 0:
                ...
                episode_trace.append({
                    ...
                    "reward_type": str(info.get("reward_type", "")),
                    "allocation_fail": info.get("allocation_fail", {}),
                })
        ...
        if e % LOG_EVERY == 0 or e == EPISODES - 1:
            ...
            if devices:
                ...
                for t in episode_trace:
                    fail_reason = ""
                    alloc_fail = t.get("allocation_fail") or {}
                    if isinstance(alloc_fail, dict) and alloc_fail.get("reason"):
                        fail_reason = f" | FAIL: {str(alloc_fail.get('reason'))}"
                    print(
                        f"   L{int(t['layer']):02d} -> D{int(t['device'])} | "
                        f"{float(t.get('t_comp',0)):6.3f} {float(t.get('t_comm',0)):6.3f}"
                        f"{fail_reason}"
                    )
