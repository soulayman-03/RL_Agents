    ...
    *** Device-Specific Power Model ***
    ...
    def __init__(
        ...
        *,
        ...
        sequential_diversity: bool = True,
    ):
        super().__init__()
        ...
        self.sequential_diversity = bool(sequential_diversity)
        ...
        try:
            self.resource_manager.sequential_diversity = self.sequential_diversity
        except AttributeError:
            pass
    ...
    def _device_energy_cost(self, device_id: int, t_comp: float, t_comm: float) -> float:
        """
        Device-specific energy model: E = P_i * T

        E_comp = power_comp_i * t_comp   (CPU active during computation)
        E_comm = power_comm_i * t_comm   (NIC active during transmission)

        power_comp_i and power_comm_i are set per-device in _assign_device_powers().
        """
        ...
        if dev is None:
            ...
            p_comp = float(getattr(dev, "power_comp", self.base_power_comp))
    ...
    def get_valid_actions(self) -> Dict[int, List[int]]:
        ...
        for agent_id in range(self.num_agents):
            ...
            for device_id in range(self.num_devices):
                ...
                t_comp_service = float(getattr(current_layer, "computation_demand", 0.0)) / float(getattr(dev_obj, "cpu_speed", 1.0))
                ...
                # Energy hard constraint — device-specific model: E = P_i * T
                cost = self._device_energy_cost(device_id, t_comp_service, t_comm_service)
    ...
    def step(self, actions: Dict[int, int]):
        ...
        for aid in agent_ids:
            ...
            # Energy cost — device-specific model: E = P_i * T
            cost = float(self._device_energy_cost(selected_device_id, comp_latency, trans_latency))
            ...
            infos[aid] = {
                ...
                "energy_comp":      float(getattr(dev, "power_comp", 0.0) * comp_latency),
